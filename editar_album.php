...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$album_id || !$titulo) {
        ...
        header('Location: index.php?status=erro_validacao');
        exit;
    }
    ...
    try {
        ...
        if ($situacao_id == $ID_SITUACAO_ADQUIRIDO) {
            ...
            $url_redirecionamento = "adicionar_colecao.php?store_id=" . $album_id;
        ...
        // ------------------------------------------------------
        header('Location: index.php?status=editado&album=' . urlencode($titulo));
        exit;

    } catch (\PDOException $e) {
        // Se a edição falhar, volta para o index com erro.
        header('Location: index.php?status=erro_db&msg=' . urlencode($e->getMessage()));
        exit;
    }
} 
...
<div class="container">
    <div class="form-container" style="max-width: 600px; margin-top: 40px;">
        ...
        <h1><?php echo $album ? 'Editar Álbum: ' . htmlspecialchars($album['titulo']) : 'Erro'; ?></h1>
        ...
        <?php if ($album): ?>
            ...
            <form method="POST" action="editar_album.php" class="edit-form">
                ...
                <input type="text" id="titulo" name="titulo" required 
                       value="<?php echo htmlspecialchars($album['titulo']); ?>">
